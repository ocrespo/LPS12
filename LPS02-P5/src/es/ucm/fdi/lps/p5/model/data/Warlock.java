...
import es.ucm.fdi.lps.p5.model.data.Cost.PayCost;
...
import es.ucm.fdi.lps.p5.model.data.actions.permanent.PermanentAction;
...
import es.ucm.fdi.lps.p5.model.game.Observed;
...
public class Warlock extends Observed implements Serializable {
	/**
	 * 
	 */
	...
	public PlayCard playCard(Card c,boolean canEnvi,boolean canInCombat){
		...
		if(value == PlayCard.goEnviroment){
			...
			hand.removeCard(c);
			super.setChanged();
			super.notifyPlayCardMana(this);
			
			return value;
		}
		else if(value == PlayCard.goGame){
			PayCost enought = mana.pay(c.getCost());
			if(enought == PayCost.allOk){
				inGame.insert(c);
				hand.removeCard(c);
				super.setChanged();
				super.notifyPlayCardEntity(this);
				return value;
			}
	...
	public boolean payDeal(Element e,Card c,boolean isEntity){
		boolean finishDeal = mana.payDealMana(e);
	...
	public boolean found(Card c){
		return inGame.found(c) || enviroment.found(c);
	}
	/**
	 * 
	 * @return size of deck
