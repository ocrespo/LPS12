...
import es.ucm.fdi.lps.p5.DAO.DAO;
...
public class Configuration {
	 ...
	 */
	public boolean readDeck(String dir,Warlock w) throws IOException,FileError,Exception{
		...
		while((line=br.readLine())!=null && pass){
			if((!line.equals("")) && (!line.startsWith("#")) && (!line.equals("@admin"))){
				if (!title){
					if(line.startsWith("BRUJOS&TRIFULCAS")){
						if (!line.endsWith("5") && !line.endsWith("4") && !line.endsWith("3") && !line.endsWith("2") && !line.endsWith("1")){
		...
		if(actAdmin){
			w.addMana(new Cost(10, 10, 10, 10, 10));
			w.setLife(9999999);
	...
	private boolean parserCard(boolean pass,String line,Warlock w,boolean actAdmin) throws DAOError, FileError{
		...
		if(code.equals("@mano") && actAdmin){
			...
			w.addInHand(CardFactory.getCard(dto));
			return true;
		}
		...
		if(code.equals("@entorno") && actAdmin){
			dto = (DAO.getInstance()).getCard(tok.nextToken());
			w.addEnviroment(CardFactory.getCard(dto));
			return true;
		}
		...
		if(dto != null){
			Card c = CardFactory.getCard(dto); // CardFactory.getCard(code);
			if(w.foundInDeck(code) == null || actAdmin || debug){
				...
				if(max == 0 || num<=max || debug){
					...
					for(int i=1;i<num;i++){
						if(actAdmin)
							w.addInGame(CardFactory.getCard(dto));
						else
							w.addCardToDeck(CardFactory.getCard(dto));
					}
				}
				else{
					if(!debug){
			...
			(Console.getInstance()).write("La carta, "+code+" del brujo '"+w.getName()+" no se reconocio como una carta valida");
			if(!console){
