...
import es.ucm.fdi.lps.p5.view.internal.CombatView;
import es.ucm.fdi.lps.p5.view.internal.ExchangeView;
import es.ucm.fdi.lps.p5.view.internal.InfoInternalFrame;
import es.ucm.fdi.lps.p5.view.internal.InterventionsSolveView;
import es.ucm.fdi.lps.p5.view.internal.ListFrame;
...
public class GameView extends JFrame implements Observer {
	 ...
	 */
	private void rotate(boolean allTurn){
		deleteGameZone();
		zoneDef.removeAll();
	 ...
	 */
 	private JPanel found(JPanel zone,Card id){
		Component[] aux = zone.getComponents();
		int pos = 0;
		int max = aux.length;
		boolean found = false;
	...
	private void showCombatFrame(boolean automatic){
		combatFrame = new CombatView(combatInfo,automatic,controller,this);
	...
	private void showExchangeFrame(JPanel zoneA,JPanel zoneB){
		exchangeFrame = new ExchangeView(zoneA,zoneB,controller,selectCard.getId());
		try {
	 ...
	 * @param id the id of card
	 */
	private void removeLink (LinkedList<CardPanel> lin,final Card id){
		int pos = 0;
	...
	public class CardPanel extends JPanel implements Cloneable{
		...
		private void iniConfigListener(){
			...
			earthButton.addActionListener(new ActionListener() {
				...
				@Override
				public void actionPerformed(ActionEvent e) {
					controller.payDealMana(Element.earth, id);
			...
			spiritButton.addActionListener(new ActionListener() {
				...
				@Override
				public void actionPerformed(ActionEvent e) {
					controller.payDealMana(Element.spirit, id);
			...
			seaButton.addActionListener(new ActionListener() {
				...
				@Override
				public void actionPerformed(ActionEvent e) {
					controller.payDealMana(Element.sea, id);
			...
			airButton.addActionListener(new ActionListener() {
				...
				@Override
				public void actionPerformed(ActionEvent e) {
					controller.payDealMana(Element.air, id);
		...
		private void buildCost(JPanel cost){
			if(id.getCost() != null){
				int earth = id.getCost().getEarth();
				int air = id.getCost().getAir();
				int sea = id.getCost().getSea();
				int spirit = id.getCost().getSpirit();
				int generic = id.getCost().getGeneric();
				if( earth > 0){
		 ...
		 */
		private void entityCardActionPanel(){
			attack.setVisible(true);
			attack.setFocusable(true);
			
			if(!id.haveActiveAction()){
				use.setVisible(false);
				use.setFocusable(false);
				setSeparate();
			}
		 ...
		 */
		private void manaCardActionPanel(){
			if(id.haveActiveAction()){
				setUseActionInGame();
			}
			else{
				...
				if(selectCard == null && canUse && discardFrame == null){
					...
					if(selectCard != null){
						if(select == Select.play){
							targetCard = card;
							controller.playCard(selectCard.getId(), id);
						}
						else if(select == Select.use){
							targetCard = card;
							controller.useAction(selectCard.getId(), id);
						}
						else if(select == Select.defense){
							targetCard = card;
							controller.defense(id, selectCard.getId());
						}
					}
				}
	...
	public class WarlockPanel extends JPanel{
		...
		public class MouseActionWarlock implements MouseListener{
			...
			public void mouseClicked(MouseEvent arg0) {
				if(selectCard != null){
					if(select == Select.play){
						controller.playCardWarlock(id);
						controller.playCard(selectCard.getId(), null);
					}
				}
	...
	public class PanelBackgroundImage extends JPanel{
		...
		public void paint(Graphics g) {	
	        try {
				...
				//g.drawImage(ImageIO.read(new File(dir)), 0, 0, getWidth(), getHeight(),this);
			} catch (IOException e) {
				e.printStackTrace();
			}
	...
	public void updateNewHandCard(final Card id,final Warlock w) {
		if(w == ((WarlockPanel) waInfo).getId()){
	...
	public void updatePlayCardMana(final Warlock w) {
		...
		selectCard.manaCardActionPanel();
		selectCard = null;
	...
	public void updatePlayCardEntity(final Warlock w) {
		...
		selectCard.defaultCard();		
		selectCard.entityCardActionPanel();
		selectCard = null;
	}
	@Override
	...
	public void updatePlayCardIntervention() {
		...
		selectCard.updateStats(String.valueOf(selectCard.id.getAttack()),String.valueOf(selectCard.id.getDefense()),String.valueOf(selectCard.id.getLife()),selectCard.id.getBuff(),selectCard.id.getDescription(),!selectCard.id.canUse(),selectCard.id.haveActiveAction(),false);
	...
	public void updateStatsCard(final Card id) {
		...
		else if((aux = found(gameB,id)) != null){
			((CardPanel) aux).updateStats(String.valueOf(id.getAttack()),String.valueOf(id.getDefense()),String.valueOf(id.getLife()),id.getBuff(),id.getDescription(),!id.canUse(),id.haveActiveAction(),false);
			aux.validate();
		}
		if(aux == null){
			...
			else if((aux = found(manaB,id)) != null){
				((CardPanel) aux).updateStats(String.valueOf(id.getAttack()),String.valueOf(id.getDefense()),String.valueOf(id.getLife()),id.getBuff(),id.getDescription(),!id.canUse(),id.haveActiveAction(),false);
				aux.validate();
			}
		}
	...
	public void updateRemoveManaCard(final Card c,final Warlock w) {
		if(w == ((WarlockPanel) waInfo).getId()){		
			manaA.remove(found(manaA, c));
			manaA.revalidate();
		
		}
		else{
			manaB.remove(found(manaB, c));
			manaB.revalidate();
		}	
	...
	public void updateRemoveHandCard(final Card c,final Warlock w) {
		if(w == ((WarlockPanel) waInfo).getId()){
			handA.remove(found(handA, c));
			((WarlockPanel)waInfo).refreshStats(String.valueOf(w.getName()), String.valueOf(w.getNumHand()),String.valueOf(w.getNumGraveyard()) ,String.valueOf( w.getNumDeck()), String.valueOf(w.getLife()),w.getInfoBuff());

		}
		else{
			handB.remove(found(handB, c));
			((WarlockPanel)wbInfo).refreshStats(String.valueOf(w.getName()), String.valueOf(w.getNumHand()),String.valueOf(w.getNumGraveyard()) ,String.valueOf( w.getNumDeck()), String.valueOf(w.getLife()),w.getInfoBuff());
	...
	public void updateInsertNewCombat() {
		...
		combatInfo.put(selectCard.getId(), lin);
		selectCard.defaultCard();
		selectCard = null;
		targetCard = null;
	...
	public void updateInsertInCombat() {
		combatInfo.get(targetCard.getId()).add(selectCard);
		selectCard.image.setCombat(true);
		selectCard.defaultCard();	
		selectCard = null;
		targetCard = null;
	...
	public void updateRemoveFromCombat(Card at, Card c) {
		JPanel aux = found(gameA, c);
		if(aux != null){	
			removeLink(combatInfo.get(at),c);
	...
	public void updateRemoveIntervention(Card c) {
		if(interventionFrame != null){
			JPanel aux = found(interventionZone, c);
	...
	public void updateInsertNewCombat(Card c, Warlock w) {
		...
		CardPanel aux = null;
		if(w == ((WarlockPanel) waInfo).getId()){
			aux = (CardPanel) found(gameA, c);
		}
		else{
			aux = (CardPanel) found(gameB, c);
		}
		LinkedList<CardPanel> lin = new LinkedList<CardPanel>();
		lin.add(aux);
		aux.image.setCombat(true);
		combatInfo.put(aux.getId(), lin);
	...
	public void updateInsertInCombat(Card at,Card def, Warlock w) {
		CardPanel aux = null;
		if(w == ((WarlockPanel) waInfo).getId()){
			aux = (CardPanel) found(gameA, def);
		}
		else{
			aux = (CardPanel) found(gameB, def);
		}
