...
public class EnviromentExchange implements ActiveAction{
	...
	public ErrorMen allOk(TreeCard combat, Card tar,Card self, Warlock w1, Warlock w2,Warlock wObj, StackCard interventions) {
		...
		Card auxTar1 = w1.foundInEnviromentCard(tar);
		Card auxTar2 = w2.foundInEnviromentCard(tar);
		if(auxTar1 == null && auxTar2 == null){
			return new ErrorMen(ErrorEnum.needTargetEnviroment);
		}
		if(lin1 == null){
			lin1 = new LinkedCard();
		...
		//Card auxTar1 = lin1.found(tar);
		//Card auxTar2 = lin2.found(tar);
		if(lin1.foundCard(tar) != null || lin2.foundCard(tar) != null){
			return new ErrorMen(ErrorEnum.targetAlreadySelect);
		}
		if((lin1.size() == w2.getNumEnoviroments() && auxTar1 != null)|| ( lin2.size() ==  w1.getNumEnoviroments() && auxTar2 != null)){
			return new ErrorMen(ErrorEnum.cantExchangeMoreEnviroment);
		}
		if(auxTar1 != null){
			lin1.insert(auxTar1);
		}
		else if(auxTar2 != null){
			lin2.insert(auxTar2);
		}
